...
class TLObject:
    ...
    def serialize_bytes(data):
        ...
        data_len = len(data)
        padding = (4 - (data_len + 1) % 4) % 4 if data_len < 254 else (4 - data_len % 4) % 4
        if data_len < 254:
            return bytes([data_len]) + data + b'\x00' * padding
        header = bytes([254,data_len & 0xFF,(data_len >> 8) & 0xFF,(data_len >> 16) & 0xFF])
